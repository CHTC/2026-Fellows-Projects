...
BASE_DIR = "mc_runs_100000"
GRAPH_DIR = "graphs/100000_jobs"
...
fig.suptitle(
    f"Monte Carlo π Convergence — All Runs ({len(all_runs)} clusters, 100,000 jobs each)",
...
plt.tight_layout()
scatter_path = os.path.join(GRAPH_DIR, "mc_runs_100000_scatter.png")
...
fig2.suptitle(
    f"Runtime vs. Number of Samples — 10 samples/job ({len(all_runs)} cluster(s))",
...
plt.tight_layout()
runtime_path = os.path.join(GRAPH_DIR, "mc_runs_100000_runtime.png")
...
fig4.suptitle(
    f"Runtime vs. Number of Jobs — 100,000 jobs ({len(all_runs)} cluster(s))",
    fontsize=13, fontweight="bold",
)
for i, (cluster_id, df) in enumerate(all_runs.items()):
    t0 = df["timestamp"].iloc[0]
...
plt.tight_layout()
runtime_jobs_path = os.path.join(GRAPH_DIR, "mc_runs_100000_runtime_jobs.png")
...
if turnaround_data:
    ...
    plt.tight_layout()
    turnaround_path = os.path.join(GRAPH_DIR, "mc_runs_100000_turnaround.png")
