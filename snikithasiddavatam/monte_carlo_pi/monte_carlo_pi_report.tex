...
\item
  Sleeps for a random duration (1--60s, seeded reproducibly) before
  exiting, to stagger completion times
\end{itemize}
...
Submits \emph{J} independent jobs to HTCondor (\texttt{queue\ J}), each
running
\texttt{mc\_pi.py\ \textless{}S\textgreater{}\ \$(Process)\ \$(Cluster)}.
Output, error, and log files are organized per-cluster under
\texttt{run\_\$(Cluster)/logs/}, and result files are transferred back
and remapped into the same folder.
...
\section{Next Steps: Memory
Profiling}\label{next-steps-memory-profiling}
