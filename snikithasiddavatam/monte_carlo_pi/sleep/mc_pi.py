...
def main():
    # Argument Parsing
    if len(sys.argv) != 5:
        print("Usage: python mc_pi.py <S> <ProcID> <ClusterID> <sleep_time>", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        sleep_time = float(sys.argv[4]) # Sleep time in seconds
    ...
    print(f"Job {proc_id}: cluster={cluster_id}, seed={seed}, S={S}, M={M}, sleep_time={sleep_time} → wrote {output_filename}")

    # sleep for the duration assigned in sleep.csv to stagger job completion times
    print(f"Job {proc_id}: sleeping for {sleep_time:.2f}s before exiting")
